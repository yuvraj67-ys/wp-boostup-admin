...
export default function ManageUsers() {
  ...
  if (filter === 'premium') {
    displayedUsers = users.filter((u) => u.isPremium);
  }
          ...
          <tbody>
            {displayedUsers.map((user) => (
              ...
              >
                ...
                {/* UPDATED PREMIUM COLUMN */}
                <td className="p-4">
                  {user.isPremium ? (
                    <span className="inline-flex flex-col items-start bg-yellow-100 text-yellow-700 px-2.5 py-1 rounded-md text-xs font-bold">
                ...
                <td className="p-4 flex gap-2">
                  <button
                    onClick={() =>
                      togglePremium(user.id, user.isPremium)
                    }
                    className="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-blue-100 border border-blue-100"
                  >
                    {user.isPremium
                      ? 'Remove Premium'
                      : 'Make Premium'}
                  ...
                  <button
                    onClick={() =>
                      toggleBanStatus(user.id, user.status)
                    }
                    className="bg-gray-50 text-gray-600 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-gray-200 border border-gray-200"
                  >
                    {user.status === 'active'
                      ? 'Ban'
                      : 'Unban'}
